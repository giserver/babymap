    ...
    c.plugins?.forEach((p: any) => {
      if (p["name"] === "vitepress:data") {
        ...
        p.load = async (id: string) => {
          if (id.endsWith("KHR_animation_pointer.data.js")) return;
...
export default {
  ...
  themeConfig: {
    ...
    socialLinks: [
      { icon: "github", link: "https://github.com/giserver/babymap" },
    ],
  },
  vite: {
